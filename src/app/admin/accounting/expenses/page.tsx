  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-white/60 text-sm">Track clinic costs & overheads</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="bg-card border border-white/10 rounded-xl p-5 flex items-center justify-between">
        <span className="text-white/60 text-sm">Total Expenses — {periodLabel(period)}</span>
        <span className="text-2xl font-bold text-red-400">{formatUSD(total)}</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-white/40">No expenses for {periodLabel(period)}</div>
      ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/10">
                ...
                <th className="text-left text-white/60 text-xs uppercase tracking-wider px-5 py-3">Category</th>
            ...
            <tbody>
              {expenses.map((exp) => (
                <tr key={exp.id} className="border-b border-white/5 hover:bg-white/5">
                  <td className="px-5 py-3 text-white/70 text-sm">{exp.date}</td>
                  <td className="px-5 py-3 text-white text-sm">{exp.description}</td>
                  <td className="px-5 py-3">
                    <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full bg-white/10 text-white/60">{exp.category}</span>
                  </td>
                  <td className="px-5 py-3 text-right text-red-400 font-medium text-sm">{formatUSD(exp.amount)}</td>
                  <td className="px-5 py-3 text-right">
                    <div className="flex items-center gap-1 justify-end">
                      <button onClick={() => { setEditing(exp); setIsNew(false); }} className="p-1.5 hover:bg-white/10 rounded-lg">
            ...
            <tfoot>
              <tr className="bg-white/5">
                <td colSpan={3} className="px-5 py-3 text-white font-semibold text-sm">TOTAL</td>
                <td className="px-5 py-3 text-right text-red-400 font-bold text-sm">{formatUSD(total)}</td>
                <td></td>
              </tr>
            </tfoot>
          </table>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-white/10 rounded-xl p-6 max-w-md w-full">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-white font-semibold">{isNew ? "Add Expense" : "Edit Expense"}</h3>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-white/60 text-xs uppercase tracking-wider mb-1 block">Category</label>
